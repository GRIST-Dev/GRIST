...
!----------------------------------------------------------------------------
 ...
 module grist_hpe_vertical_advection
   ...
   use grist_nml_module,     only: eqs_vert_diff
...
!-----------------------------------------------------------------------
   ...
   subroutine calc_hpe_vert_advection(nlev, nlevp                        ,&
...
!
     ...
     real(r8),   intent(in)    :: scalar_mass_eta_velocity_face_level(:) ! m*etadot 
     real(r8),   intent(in)    :: scalar_delhp_full_level(:)
     real(r8),   intent(in)    :: scalar_at_full_level(:)
     real(r8),   intent(inout) :: scalar_vert_advection_full_level(:)    ! with minus sign
!
! local
!
     integer(i4)                          :: ilev
...
!
     call calc_hpe_vert_flux_operator(nlev, nlevp, scalar_at_full_level, scalar_mass_eta_velocity_face_level ,&
                                      2, scalar_at_face_level, .true.)
...
!
#ifdef REFCODE
   subroutine calc_hpe_tend_vert_mass_flux(nlev, nlevp,scalar_at_full_level , &
                                           scalar_eta_velocity  , &
                                           order                , &
                                           tend_vert_mass_flux)
! io
...
! local
    ...
    integer(i4)                               :: ilev
     ...
     call calc_hpe_vert_flux_operator(nlev, nlevp,scalar_at_full_level   ,&
                                  scalar_eta_velocity    ,&
                                  order                  ,&
   ...
   subroutine calc_hpe_tend_vert_mass_flux_2d(ncell, ncell_do, nlev, nlevp, &
                                              scalar_at_full_level , &
                                              scalar_eta_velocity  , &
                                              order                , &
                                              tend_vert_mass_flux)
! io
    use omp_lib
    integer(i4),  intent(in)                         :: ncell, ncell_do, nlev, nlevp
...
!$omp do schedule(dynamic,5) 
     do icell = 1, ncell_do
        call calc_hpe_vert_flux_operator(nlev, nlevp,scalar_at_full_level(1:nlev,icell)   ,&
...
!-----------------------------------------------------------------------
   ...
   subroutine calc_hpe_vert_flux_operator(nlev, nlevp, scalar_at_full_level    ,&
...
! 
     ...
     scalar_at_face_level(1)        = zero
     scalar_at_face_level(nlev+1)   = zero
...
! face level
           part1 = (scalar_at_full_level(ilev)*deta_full(ilev-1)+scalar_at_full_level(ilev-1)*deta_full(ilev))*half/deta_face(ilev)
! full level for this face level
         ...
         !                                       (scalar_at_full_level(ilev-1)-scalar_at_full_level(ilev-2))/deta_face(ilev-1))/(deta_face(ilev)+deta_face(ilev-1))
         !der_kp1= 2._r8*(deta_full(ilev)**2)*((scalar_at_full_level(ilev+1)-scalar_at_full_level(ilev))/deta_face(ilev+1)-&
         !                                     (scalar_at_full_level(ilev)-scalar_at_full_level(ilev-1))/deta_face(ilev))/(deta_face(ilev+1)+deta_face(ilev))
         !scalar_at_face_level(ilev)= part1-(der_k+der_kp1)/12._r8+sign(1._r8,scalar_mass_eta_velocity(ilev))*(der_kp1-der_k)/12._r8

         ! 
           der_k  = (deta_full(ilev-1)**2)*((scalar_at_full_level(ilev)-scalar_at_full_level(ilev-1))/deta_face(ilev)-&
                                            (scalar_at_full_level(ilev-1)-scalar_at_full_level(ilev-2))/deta_face(ilev-1))/(deta_face(ilev)+deta_face(ilev-1))
           der_kp1= (deta_full(ilev)**2)*((scalar_at_full_level(ilev+1)-scalar_at_full_level(ilev))/deta_face(ilev+1)-&
                                          (scalar_at_full_level(ilev)-scalar_at_full_level(ilev-1))/deta_face(ilev))/(deta_face(ilev+1)+deta_face(ilev))

           scalar_at_face_level(ilev)= part1-(der_k+der_kp1)/6._r8+sign(1._r8,scalar_mass_eta_velocity(ilev))*(der_kp1-der_k)/6._r8
        end do
     case default
        print*, "you must select a vertical order, stop"
        stop
...
! partial m*etadot
! NHD now has bespoke version
!-----------------------------------------------------------------------

   subroutine calc_hpe_tend_vert_mass_flux_face(nlev, nlevp, scalar_at_face_level      , &
...
! local
    ...
    integer(i4)                               :: ilev
     ...
     call calc_hpe_vert_flux_operator_face(nlev, nlevp, scalar_at_face_level     ,&
                                       scalar_eta_mass_velocity_full ,&
                                       order                    ,&
                                       scalar_at_full_level )
...
!
   ...
   subroutine calc_hpe_vert_flux_operator_face(nlev, nlevp, &
                                               scalar_at_face_level    ,&
                                               scalar_eta_mass_velocity_full,&
                                               order                   ,&
                                               scalar_at_full_level    )
    ...
    real(r8)                            :: part1, der_k, der_kp1
     ...
     case(3)
        ...
        do ilev = 2, nlev-1
           ...
           scalar_at_full_level(ilev) = part1-(der_k+der_kp1)/12._r8+sign(1._r8,scalar_eta_mass_velocity_full(ilev))*(der_kp1-der_k)/12._r8
        end do
