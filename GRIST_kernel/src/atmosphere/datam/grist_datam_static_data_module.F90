...
!
    ...
    use grist_nml_module,   only: nlev, staticFilePath, sftopoFilePath, sstFilePath, aqua_sst_style, real_sst_style, numMonSST, test_real_case, topo_type, &
                                  ...
                                  psmeanFilePath, use_psmean_hpgf
...
! psmean
   type(scalar_1d_field), public   :: staticData_psmean_at_pc_surface   !
! land-surface
...
!--------------
    ...
    if(use_psmean_hpgf)then
       call wrap_allocate_data1d(mesh%nv, staticData_psmean_at_pc_surface)
       call wrap_read_1d_group_rst(mesh%gcomm_read, trim(psmeanFilePath), '', 'PSMEAN', 0, staticData_psmean_at_pc_surface%f)
    end if
...
! static gwdo
   ...
   subroutine grist_static_fill_current_sst(mesh,nstep,ncell,dtime,is_first_restart)
    ...
    use grist_wrap_nf,                   only: wrap_open, wrap_inq_dimid, wrap_inq_dimlen
!io
...
!================================================
     ...
     case('AMIP')
       if(nstep.eq.0.or.mod(year,10).eq.0.or.is_first_restart)then
          call write_amip_date
          sstFileName = trim(sstFileNameHead)//trim(char_year)//trim(sstFileNameTail)
       end if
     case('DAILY')
        call write_daily_date
        sstFileName = trim(sstFileNameHead)//'daily'//trim(char_year)//trim(char_mon)//trim(char_day)//trim(sstFileNameTail)
     case default
        sstFileName = trim(sstFileNameHead)//'cycle'//trim(sstFileNameTail)
     end select

     !call barrier(grist_comm())
     if(mpi_rank().eq.0)then
        write(6,*) "if abort, please ensure", trim(sstFilePath)//trim(sstFileName), " has a dimension called 'time'"
        call wrap_open(trim(sstFilePath)//trim(sstFileName), 0, nfid)
        call wrap_inq_dimid (nfid, 'time', dimid)
        call wrap_inq_dimlen(nfid, dimid, dimlen_time)
        write(6,*) "INFO: Time dimension length of",trim(sstFileName)," file is",dimlen_time
     end if
     !call barrier(grist_comm())
     CALL mpi_bcast(dimlen_time,1,MPI_INTEGER,0,grist_comm(),ierr)
...
!================================================
    ...
    case('CYCLE')
     ...
     if(numMonSST.ne.12.or.numMonSST.ne.dimlen_time)then
        print*,"for cycle sst, numMonSST must be 12; must have a 'time' dimension as unlimited"
        call mpi_abort()
     end if
...
!==================================================================================================
     ...
     ! safe check
     !================================================  
        if(mod(sstFile_year_beg,10).ne.0)then
          print*, "for AMIP (non-cycle) sst/sic, sstFile_year_beg must be tailed by zero"
          call mpi_abort()
        end if

        dim_2  = numMonSST  ! must be 122

        if(numMonSST.ne.122.or.numMonSST.ne.dimlen_time)then
           print*, "for AMIP (non-cycle) sst/sic, numMonSST in nml must be 122=1+12*10+1; must have a 'time' dimension as unlimited"
           call mpi_abort()
        end if
...
!
      ...
      if(numMonSST.ne.1.or.numMonSST.ne.dimlen_time)then
         print*, "for daily sst, numMonSST must be 1; must have a 'time' dimension as unlimited"
         call mpi_abort()
      end if
  ...
  contains
     ...
     end subroutine write_daily_date 
