...
! Declare the command string
     character(len=9999) :: command1, command2,bdyDataPath
     integer :: istat
...
! glamData_merge_online
!================================================
     bdyDataPath = trim(glamAtmFilePath)

     If(glamData_merge_online)then
! Define the command to be executed
command1 = trim('ncrcat '//trim(glamAtmFilePath)//'GRIST.lamData.base.nc '//trim(glamAtmFilePath)//'GRIST.lamData.'//trim(char_t1_year)//trim(char_t1_mon)//trim(char_t1_day)//trim(char_t1_sec)//'.nc ./GRIST.lamData.'//trim(char_t1_year)//trim(char_t1_mon)//trim(char_t1_day)//trim(char_t1_sec)//'.nc')

command2 = trim('ncrcat '//trim(glamAtmFilePath)//'GRIST.lamData.base.nc '//trim(glamAtmFilePath)//'GRIST.lamData.'//trim(char_t2_year)//trim(char_t2_mon)//trim(char_t2_day)//trim(char_t2_sec)//'.nc ./GRIST.lamData.'//trim(char_t2_year)//trim(char_t2_mon)//trim(char_t2_day)//trim(char_t2_sec)//'.nc')


! Call the system command
        if(mpi_rank().eq.0)then
           call execute_command_line(trim(command1), exitstat=istat)
           if(istat /= 0) then
              write(6,*) 'Error: Command failed with status:', istat
           else
              write(6,*) 'Command1 executed successfully.'
           end if
        end if

        if(mpi_rank().eq.1)then
           call execute_command_line(trim(command2), exitstat=istat)
           if(istat /= 0) then
              write(6,*) 'Error: Command failed with status:', istat
           else
              write(6,*) 'Command2 executed successfully.'
           end if
        end if
...
!================================================
     ...
     call wrap_read_2d_group_rst(mesh%gcomm_read, trim(bdyDataPath),&
     ...
     call wrap_read_3d_group_rst(mesh%gcomm_read, trim(bdyDataPath),&
          'GRIST.lamData.'//trim(char_t1_year)//trim(char_t1_mon)//trim(char_t1_day)//trim(char_t1_sec)//'.nc',&
...
!
     case('ERA_ml','MERRA_ml','ERA_Pres','GFS_Pres')
        ...
        call wrap_read_1d_group_rst(mesh%gcomm_read, trim(bdyDataPath),&
          'GRIST.lamData.'//trim(char_t1_year)//trim(char_t1_mon)//trim(char_t1_day)//trim(char_t1_sec)//'.nc',&
...
!
     If(glamData_merge_online)then
        call barrier(grist_comm())
        command1 = trim('rm -rf ./GRIST.lamData.'//trim(char_t1_year)//trim(char_t1_mon)//trim(char_t1_day)//trim(char_t1_sec)//'.nc')
        command2 = trim('rm -rf ./GRIST.lamData.'//trim(char_t2_year)//trim(char_t2_mon)//trim(char_t2_day)//trim(char_t2_sec)//'.nc')
